...
impl ToolHandler for UnblockTaskHandler {
  ...
  async fn handle_async(
    ...
    let args: UnblockTaskArgs = invocation.parse_arguments()?;
    let runtime = invocation.runtime.ok_or_else(|| {
      FunctionCallError::Fatal("unblock_task missing runtime context".to_string())
    })?;
    let team_runtime = runtime_for_thread(&runtime.thread_id).ok_or_else(|| {
      FunctionCallError::Execution("unblock_task runtime is not configured".to_string())
    })?;
    let task = team_runtime
      .unblock_task(&args.task_id, args.blocker_id.as_deref())
      .await
      .ok_or_else(|| {
        FunctionCallError::RespondToModel(format!("failed to unblock task {}", args.task_id))
      })?;
    ...
    let out = ToolOutput::success(serde_json::to_string(&task).map_err(|err| {
      FunctionCallError::Fatal(format!("failed to serialize unblocked task: {err}"))
    })?);
