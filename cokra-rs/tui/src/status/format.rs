// 1:1 codex: FieldFormatter for aligned label-value pairs in status cards.
...
mod tests {
  ...
  fn formatter_aligns_labels() {
    let formatter = FieldFormatter::from_labels(["Model", "Directory", "Permissions"]);
    let line = formatter.line("Model", vec![Span::from("gpt-4.1")]);
  ...
  fn truncate_respects_max_width() {
    let line = Line::from("Hello, world! This is a long line.");
    let truncated = truncate_line_to_width(line, 10);
    let width = line_display_width(&truncated);
    assert!(width <= 10);
  }

  #[test]
  fn value_width_subtracts_offset() {
    let formatter = FieldFormatter::from_labels(["Model", "Token usage"]);
    let available = 60;
    let vw = formatter.value_width(available);
    assert!(vw < available);
    assert!(vw > 0);
  }
}
