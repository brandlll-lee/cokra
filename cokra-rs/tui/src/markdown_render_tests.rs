...
fn blockquote_with_ordered_list() {
  let md = "> 1. first\n> 2. second\n";
  let text = render_markdown_text(md);
  let marker = Style::new().fg(light_blue());
  let expected = Text::from_iter([
    Line::from_iter(vec![
      Span::from("> "),
      Span::styled("1. ", marker),
      Span::from("first"),
    ])
    .green(),
    Line::from_iter(vec![
      Span::from("> "),
      Span::styled("2. ", marker),
      Span::from("second"),
    ])
    .green(),
  ]);
...
fn list_ordered() {
  ...
  let marker = Style::new().fg(light_blue());
  let expected = Text::from_iter([
    Line::from_iter([Span::styled("1. ", marker), "List item 1".into()]),
    Line::from_iter([Span::styled("2. ", marker), "List item 2".into()]),
  ]);
  assert_eq!(text, expected);
}
...
fn list_ordered_custom_start() {
  ...
  let marker = Style::new().fg(light_blue());
  let expected = Text::from_iter([
    Line::from_iter([Span::styled("3. ", marker), "First".into()]),
    Line::from_iter([Span::styled("4. ", marker), "Second".into()]),
  ]);
  assert_eq!(text, expected);
}
...
fn nested_unordered_in_ordered() {
  ...
  let marker = Style::new().fg(light_blue());
  let expected = Text::from_iter([
    Line::from_iter([Span::styled("1. ", marker), "Outer".into()]),
    Line::from_iter(["    - ", "Inner A"]),
    Line::from_iter(["    - ", "Inner B"]),
    Line::from_iter([Span::styled("2. ", marker), "Next".into()]),
  ]);
  assert_eq!(text, expected);
}
...
fn nested_ordered_in_unordered() {
  ...
  let expected = Text::from_iter([
    ...
    Line::from_iter([Span::styled("    1. ", marker), "One".into()]),
    Line::from_iter([Span::styled("    2. ", marker), "Two".into()]),
    Line::from_iter(["- ", "Last"]),
  ]);
  assert_eq!(text, expected);
}
...
fn loose_list_item_multiple_paragraphs() {
  ...
  let marker = Style::new().fg(light_blue());
  let expected = Text::from_iter([
    Line::from_iter([Span::styled("1. ", marker), "First paragraph".into()]),
    Line::default(),
    Line::from_iter(["   ", "Second paragraph of same item"]),
    Line::from_iter([Span::styled("2. ", marker), "Next item".into()]),
  ]);
  assert_eq!(text, expected);
}
...
fn deeply_nested_mixed_three_levels() {
  ...
  let marker = Style::new().fg(light_blue());
  let expected = Text::from_iter([
    Line::from_iter([Span::styled("1. ", marker), "A".into()]),
    Line::from_iter(["    - ", "B"]),
    Line::from_iter([Span::styled("        1. ", marker), "C".into()]),
    Line::from_iter([Span::styled("2. ", marker), "D".into()]),
  ]);
  assert_eq!(text, expected);
}
...
fn loose_items_due_to_blank_line_between_items() {
  ...
  let marker = Style::new().fg(light_blue());
  let expected = Text::from_iter([
    Line::from_iter([Span::styled("1. ", marker), "First".into()]),
    Line::from_iter([Span::styled("2. ", marker), "Second".into()]),
  ]);
  assert_eq!(text, expected);
}
...
fn mixed_tight_then_loose_in_one_list() {
  ...
  let marker = Style::new().fg(light_blue());
  let expected = Text::from_iter([
    Line::from_iter([Span::styled("1. ", marker), "Tight".into()]),
    Line::from_iter([Span::styled("2. ", marker), "Loose".into()]),
  ]);
  assert_eq!(text, expected);
}
...
fn ordered_item_with_indented_continuation_is_tight() {
  ...
  let marker = Style::new().fg(light_blue());
  let expected = Text::from_iter([
    Line::from_iter([Span::styled("1. ", marker), "Foo".into()]),
    Line::from_iter(["   ", "Bar"]),
  ]);
  assert_eq!(text, expected);
}
...
fn inline_code() {
  ...
  let code_style = Style::new().fg(light_blue());
  let expected = Line::from_iter([
    "Example of ".into(),
    Span::styled("Inline code", code_style),
  ])
  .into();
  assert_eq!(text, expected);
}
...
fn link() {
  ...
  let link_style = Style::new().fg(light_blue()).underlined();
  let expected = Text::from(Line::from_iter([
    "Link".into(),
    " (".into(),
...
fn ordered_item_continuation_paragraph_is_indented() {
  ...
  let marker = Style::new().fg(light_blue());
  let expected = Text::from_iter([
    Line::from_iter([Span::styled("1. ", marker), "Intro".into()]),
    Line::default(),
...
fn nested_item_continuation_paragraph_is_indented() {
  ...
  let expected = Text::from_iter([
    ...
    Line::from_iter([Span::styled("2. ", marker), "C".into()]),
  ]);
  assert_eq!(text, expected);
}
