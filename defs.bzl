...
def cokra_rust_crate(
    ...
    for test in native.glob(["tests/*.rs"], allow_empty = True):
        ...
        rust_test(
            ...
            # Bazel workspace-prefixed (`cokra-rs/core/tests/...`) for snapshot parity.
            rustc_flags = rustc_flags_extra + ["--remap-path-prefix=cokra-rs="],
            rustc_env = rustc_env,
